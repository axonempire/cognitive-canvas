...
interface NeuronCell {
  ...
  // Firing state
  firing: boolean;
  fireProgress: number; // 0→1 soma→axon→terminals
  fireIntensity: number;
  refractoryTimer: number; // cooldown after firing
  // Stimulation from incoming spikes
  stimulation: number; // accumulates from incoming dendrite hits
...
const NeuralCanvas = () => {
  ...
  useEffect(() => {
    ...
    const neuronCount = Math.min(90, Math.max(40, Math.floor((window.innerWidth * window.innerHeight) / 15000)));
    ...
    for (let i = 0; i < neuronCount; i++) {
      ...
      for (let d = 0; d < dendriteCount; d++) {
        ...
        for (let b = 0; b < branchCount; b++) {
          ...
          for (let s = 0; s < subBranchCount; s++) {
            subBranches.push({ angle: randRange(-0.6, 0.6), length: randRange(10, 22) });
          }
          branches.push({
            angle: randRange(-0.5, 0.5),
            length: randRange(15, 38),
        ...
        dendrites.push({ angle: baseAngle, length: randRange(30, 55), swayPhase: Math.random() * Math.PI * 2, swaySpeed: 0.002 + Math.random() * 0.004, swayAmount: 0.015 + Math.random() * 0.03, branches });
      }
      ...
      for (let t = 0; t < terminalCount; t++) {
        terminals.push({ angle: randRange(-0.8, 0.8), length: randRange(12, 30) });
      }
      ...
      neurons.push({
        ...
        somaRadius: randRange(6, 11),
        pulsePhase: Math.random() * Math.PI * 2,
        pulseSpeed: 0.008 + Math.random() * 0.012,
        dendrites,
        axon: { angle: axonAngle, length: randRange(60, 120), terminals },
    ...
    // Helpers to get world-space positions
    ...
    // Spontaneous firing to seed activity
    let time = 0;
    ...
    const animate = () => {
      ...
      for (let i = 0; i < neurons.length; i++) {
        ...
        // Check if stimulation crosses threshold → fire
        ...
        if (n.firing) {
          ...
          if (n.fireProgress >= 1.3) {
            // Firing complete — enter refractory
            n.firing = false;
            n.fireProgress = 0;
            n.fireIntensity = 0;
            n.refractoryTimer = REFRACTORY_PERIOD;

            // Release neurotransmitters from axon terminals → find nearby dendrites
              ...
              for (const term of terminals) {
                for (const tip of tips) {
                  ...
                  if (Math.sqrt(dx * dx + dy * dy) < SYNAPSE_DIST && !connected) {
                    ...
                    connected = true; // one spike per target neuron
                  }
                }
              }
            }
      ...
      // Draw synaptic connections (faint dashed lines between nearby terminals and dendrites)
      for (let i = 0; i < neurons.length; i++) {
      ...
      // Draw synaptic spike visuals (white flash traveling)
      for (const spike of synapticSpikes) {
        if (!spike.active) continue;
      ...
      for (let ni = 0; ni < neurons.length; ni++) {
        ...
        // Compute fire glow per-section based on fireProgress
        // 0-0.3: soma lights up, 0.2-0.8: axon traversal, 0.7-1.0: terminals
        ...
        // Dendrites (receive signals - glow when stimulated)
        ...
        // Fire traveling along axon - white flash
        if (n.firing && n.fireProgress > 0.2 && n.fireProgress < 0.9) {
          ...
          flashGlow.addColorStop(0, `rgba(230,200,140,0.4)`);
          flashGlow.addColorStop(0.4, `rgba(210,175,120,0.15)`);
        ...
        if (somaFire > 0) {
          sg.addColorStop(0, `rgba(230,200,140,${somaFire * 0.15})`);
          sg.addColorStop(0.5, `rgba(210,170,100,${somaFire * 0.06})`);
        } else {
        ...
        if (somaFire > 0) {
          somaGrad.addColorStop(0, `rgba(230,210,160,${0.4 + somaFire * 0.2})`);
          somaGrad.addColorStop(0.5, `rgba(210,175,110,${0.35 + somaFire * 0.15})`);
          somaGrad.addColorStop(1, `rgba(180,140,80,${0.25 + somaFire * 0.1})`);
        } else {
        ...
        ctx.fillStyle = somaFire > 0
          ? `rgba(220,190,140,${0.2 + somaFire * 0.15})`
